...
# Create mongodb-2 instance
resource "azurerm_virtual_machine" "mongodb-2" {
  depends_on                    = ["azurerm_virtual_machine.initial-peer"]
  name                          = "${var.tag_application}-mongodb-2"
  location                      = "${var.azure_region}"
  resource_group_name           = "${azurerm_resource_group.rg.name}"
  ...
  storage_os_disk {
    name          = "${var.tag_application}-mongodb-2-osdisk"
    vhd_uri       = "${azurerm_storage_account.stor.primary_blob_endpoint}${azurerm_storage_container.storcont.name}/${var.tag_application}-mongodb-2-osdisk.vhd"
    caching       = "ReadWrite"
    create_option = "FromImage"
  }

  os_profile {
    computer_name  = "${var.tag_application}-mongodb-2"
    admin_username = "${var.azure_image_user}"
    admin_password = "${var.azure_image_password}"
  }
  ...
  tags {
    X-Name        = "${var.tag_application}-${var.habitat_origin}-mongodb-2"
    X-Dept        = "${var.tag_dept}"
    X-Customer    = "${var.tag_customer}"
    X-Project     = "${var.tag_project}"
    X-Application = "${var.tag_application}"
...
# Create mongodb-3 instance
resource "azurerm_virtual_machine" "mongodb-3" {
  depends_on                    = ["azurerm_virtual_machine.initial-peer"]
  name                          = "${var.tag_application}-mongodb-3"
  location                      = "${var.azure_region}"
  resource_group_name           = "${azurerm_resource_group.rg.name}"
  ...
  storage_os_disk {
    name          = "${var.tag_application}-mongodb-3-osdisk"
    vhd_uri       = "${azurerm_storage_account.stor.primary_blob_endpoint}${azurerm_storage_container.storcont.name}/${var.tag_application}-mongodb-3-osdisk.vhd"
    caching       = "ReadWrite"
    create_option = "FromImage"
  }

  os_profile {
    computer_name  = "${var.tag_application}-mongodb-3"
    admin_username = "${var.azure_image_user}"
    admin_password = "${var.azure_image_password}"
  }
  ...
  tags {
    X-Name        = "${var.tag_application}-${var.habitat_origin}-mongodb-3"
    X-Dept        = "${var.tag_dept}"
    X-Customer    = "${var.tag_customer}"
    X-Project     = "${var.tag_project}"
    X-Application = "${var.tag_application}"
