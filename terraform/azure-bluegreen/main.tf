...
resource "azurerm_virtual_machine" "lb" {
  depends_on                    = ["azurerm_virtual_machine.lb-blue"]
  name                          = "${var.tag_application}-lb"
  location                      = "${var.azure_region}"
  resource_group_name           = "${azurerm_resource_group.rg.name}"
  network_interface_ids         = ["${azurerm_network_interface.nic.8.id}"]
  vm_size                       = "Standard_DS1_v2"
  delete_os_disk_on_termination = true
  ...
  provisioner "habitat" {
    ...
    service {
      binds    = ["backend:haproxy.${var.group}"]
    ...
    connection {
      host     = "${azurerm_public_ip.pip.8.ip_address}"
      user     = "${var.azure_image_user}"
      password = "${var.azure_image_password}"
    }
  }
